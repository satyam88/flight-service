pipeline {
    ...
    environment {
        ...
        IMAGE_TAG = "${BRANCH_NAME}flight-service-v.1.${BUILD_NUMBER}"
        DEV_IMAGE_TAG = "dev-flight-service-v.1.${BUILD_NUMBER}"
        PREPROD_IMAGE_TAG = "preprod-flight-service-v.1.${BUILD_NUMBER}"
    }
    ...
    stages {
        stage('Build and Test for Dev') {
            ...
            stages {
                ...
                stage('Build & Tag Docker Image') {
                    steps {
                        echo "Building Docker Image: ${ECR_URL}/flight-service:${DEV_IMAGE_TAG}"
                        sh "docker build -t ${ECR_URL}/flight-service:${DEV_IMAGE_TAG} ."
                ...
                stage('Push Docker Image to Amazon ECR') {
                    steps {
                        echo "Pushing Docker Image to ECR: ${ECR_URL}/flight-service:${DEV_IMAGE_TAG}"
                        withDockerRegistry([credentialsId: 'ecr:ap-south-1:ecr-credentials', url: "https://${ECR_URL}"]) {
                            sh "docker push ${ECR_URL}/flight-service:${DEV_IMAGE_TAG}"
                        }
        ...
        stage('Tag Docker Image for Preprod and Prod') {
            ...
            steps {
                script {
                    def targetTag = BRANCH_NAME == 'preprod' ? PREPROD_IMAGE_TAG : "prod-flight-service-v.1.${BUILD_NUMBER}"
                    def sourceTag = BRANCH_NAME == 'preprod' ? DEV_IMAGE_TAG : PREPROD_IMAGE_TAG
                    def sourceImage = "${ECR_URL}/flight-service:${sourceTag}"
                    def targetImage = "${ECR_URL}/flight-service:${targetTag}"
            ...
            steps {
                script {
                    ...
                    if (configMapChanged == 0) {
                        ...
                        sh """
                            kubectl --kubeconfig=/var/lib/jenkins/.kube/config rollout restart deployment dev-flight-service-deployment -n dev
                        """
                    } else {
        ...
        stage('Deploy app to prod env') {
            ...
            steps {
                script {
                    ...
                    sh """
                        sed -i 's|<latest>|prod-flight-service-v.1.${BUILD_NUMBER}|g' ${yamlFile}
                        cat ${yamlFile} | grep prod-flight-service-v.1.${BUILD_NUMBER} || echo "Replacement failed in ${yamlFile}"
                    """
                    sh """
